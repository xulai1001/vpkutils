...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    /// <summary>
    /// The VPK data structure.
    /// </summary>
    public class VPKTagData 
        : ITaggedData
    {

        private short m_TagID = 0x5650;

        /// <summary>
        /// Little-endian "VP"
        /// </summary>
        public const short ID = 0x5650;       //  little-endian "VP"
        ...
        {
            ZipHelperStream zs = new ZipHelperStream(new MemoryStream(data, offset, count, false));
            Size = zs.ReadLELong();
            CompressedSize = zs.ReadLELong();
            CRC = zs.ReadLELong();
        }
        ...
        public byte[] GetData()
        {
            ...
            ZipHelperStream zs = new ZipHelperStream(ms);
            zs.IsStreamOwner = false;
            zs.WriteLELong(Size);
            zs.WriteLELong(CompressedSize);
            zs.WriteLELong(CRC);
            return ms.ToArray();
        }
