...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class ZipOutputStream : DeflaterOutputStream
	{
		...
		public void PutNextEntry(ZipEntry entry)
		{
			...
			if (headerInfoAvailable) {
                // here: no write crc/size
				WriteLeInt(VPKOperation.IsVPK(entry) ? 0 : (int)entry.Crc);
				if ( entry.LocalHeaderRequiresZip64 ) {
					WriteLeInt(-1);
					WriteLeInt(-1);
				}
				else {
                    if (VPKOperation.IsVPK(entry))
                    {
                        WriteLeInt(entry.IsCrypted ? ZipConstants.CryptoHeaderSize : 0);
                        WriteLeInt(0);
                    }
                    else
                    {
            ...
            // here: create a temp VPK extra field stub
            VPKOperation.PackInfo(entry);
			...
			if (entry.LocalHeaderRequiresZip64) {
				ed.StartNewEntry();
				if (headerInfoAvailable) {
                    ed.AddLeLong(VPKOperation.IsVPK(entry) ? 0 : entry.Size);
                    ed.AddLeLong(VPKOperation.IsVPK(entry) ? 0 : entry.CompressedSize);
				}
				else {
					ed.AddLeLong(-1);
					ed.AddLeLong(-1);
            ...
            if (patchEntryHeader)
            {
                extraPatchPos = ed.CurrentReadIndex;
            }
		...
		public void CloseEntry()
		{
            ...
            long old_len = curEntry.ExtraData.Length;
            VPKOperation.PackInfo(curEntry);
            if (old_len != curEntry.ExtraData.Length)
                throw new ZipException("vpk length not equal to stub");

            // here : impossible in vpk!
			if (!VPKOperation.IsVPK(curEntry) && patchEntryHeader) {
				patchEntryHeader = false;
				...
				if ( curEntry.LocalHeaderRequiresZip64 ) {
                    ...
                    WriteLeLong(VPKOperation.IsVPK(curEntry) ? 0 : curEntry.Size);
                    WriteLeLong(VPKOperation.IsVPK(curEntry) ? 0 : curEntry.CompressedSize);
				}
				else {
            ...
            //here: patch vpk info
            long tmp = baseOutputStream_.Position;
            baseOutputStream_.Seek(extraPatchPos, SeekOrigin.Begin);
                baseOutputStream_.Write(curEntry.ExtraData, 0, curEntry.ExtraData.Length);
            baseOutputStream_.Seek(tmp, SeekOrigin.Begin);

			// Add data descriptor if flagged as required
			if ((curEntry.Flags & 8) != 0) {
				WriteLeInt(ZipConstants.DataDescriptorSignature);
				WriteLeInt(VPKOperation.IsVPK(curEntry) ? 0 : unchecked((int)curEntry.Crc));
				...
				if ( curEntry.LocalHeaderRequiresZip64 ) {
                    ...
                    WriteLeInt(VPKOperation.IsVPK(curEntry) ? 0 : (int)curEntry.CompressedSize);
                    WriteLeInt(VPKOperation.IsVPK(curEntry) ? 0 : (int)curEntry.Size);
		...
		public override void Finish()
		{
			...
			foreach (ZipEntry entry in entries) {
					...
					(entry.CompressedSize >= uint.MaxValue) )
				{
                    ...
                    WriteLeInt(VPKOperation.IsVPK(entry) ? 0 : (int)entry.CompressedSize);
				}
					...
					(entry.Size >= uint.MaxValue) )
				{
                    ...
                    WriteLeInt(VPKOperation.IsVPK(entry) ? 0 : (int)entry.Size);
				}
